...
public class ReimbursementRepoImpl implements IReimbursementRepo {
	...
	public int insertReimbursement(Reimbursement rObj) {
		...
		int result = jdbcTemplate.update(query, new Object[] { rObj.getUserId(), rObj.getReason(), rObj.getAmount(),
				"PENDING", DateConversion.convertDateFromUtilToSql(new Date())});
		return result;
	}
	...
	public List<Reimbursement> getAllReimbursements(int userId) {
		String query = "select r.reimbursementId,r.userId,r.reason,r.amount,r.status,\r\n"
				+ "r.raisedOn,u.userName from reimbursement_info r join user_tab u on r.userid and r.userId = ?";
		List<Reimbursement> reimbursements = jdbcTemplate.query(query, new Object[] { userId },
				new ReimbursementRowMapper());
		return reimbursements;
	}

	public int updateReimbursement(Reimbursement rObj) {
		String query = "update reimbursement_info set status = ? where reimbursement = ?";
